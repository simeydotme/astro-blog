...
This technique works because when we are in-painting, or panning an image, Midjourney (at least for now) looks at the current
...
> So this means that
if the image is only a single character but with multiple versions and poses, Midjourney infers that it should try to
...
[<MJ text="Let's Get Started!" style="next" />](/character/generating-a-base)